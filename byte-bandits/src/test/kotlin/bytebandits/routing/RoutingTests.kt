...
class RoutingTests {
	...
	fun `jwt should work`() {
		withTestApplication(Application::module) {
			handleRequest(HttpMethod.Post, "jwt/") {
				...
				addHeader("Authorization", "Bearer $sampleJWT")
				val obj = WebJWTRequest("test", true)
				val gson = Gson()
	...
	@Test
	fun `invalid jwt should fail`() {
		withTestApplication(Application::module) {
			handleRequest(HttpMethod.Post, "jwt/") {

				addHeader("Authorization", "Bearer $sampleJWT")
				val obj = "this is a bad post"
				val gson = Gson()
				addHeader("Content-Type", "application/json")
				setBody(gson.toJson(obj))
			}.apply {
	...
	@Test
	fun `bad submit should not work`() {
		withTestApplication(Application::module) {
	...
	@Test
	fun `bad delete should not work`() {
		withTestApplication(Application::module) {
			handleRequest(HttpMethod.Get, "storage/Delete/file_does/not_exist") {
				addHeader("Authorization", "Bearer $sampleJWT")
			}.apply {
				assertEquals(HttpStatusCode.BadGateway, response.status())
			}
	...
	@Test
	fun `password wallet  retrieve only should fail`() {
		withTestApplication(Application::module) {
			handleRequest(HttpMethod.Get, "/pwdWallet/Retrieve/exampsadfle.com/userID1sadf23") {
				addHeader("Authorization", "Bearer $sampleJWT")
			}.apply {
				assertEquals(HttpStatusCode.BadGateway, response.status())
			}
	...
	@Test
	fun `virus checker bad requests should fail`() {
		withTestApplication(Application::module) {
			handleRequest(HttpMethod.Post, "/virusChecker/CheckFile/") {
				// Set up headers and body for the request
				addHeader("Content-Type", "application/json")
				...
				val sampleFileContent = Base64.getEncoder().encodeToString("sample file content".toByteArray())
				val fileRequest = WebSimpleFileRequest(
					sampleFileContent + sampleFileContent.subSequence(0, 2),
					"userID123",
					"testFile.txt"
				)
				val gson = Gson()
	...
	@Test
	fun `gen password bad request`() {
		withTestApplication(Application::module) {
			handleRequest(HttpMethod.Get, "password/Get/19asdf").apply {
				assertEquals(HttpStatusCode.BadGateway, response.status())
			}
		}
	}
	...
	@Test
	fun `gen passkey bad request`() {
		withTestApplication(Application::module) {
			handleRequest(HttpMethod.Get, "passKey/Get/134sdfad/").apply {
				assertEquals(HttpStatusCode.BadGateway, response.status())
			}
		}
	}
